# Update to the latest version of phenix.  You can then run
# phenix.python ringer_ne.py PDB.pdb PDB.mtz and it should just work!
# The script below can also be run to give you a demo
# pipeline.  This will dump out a .csv file that is super easy to parse
# or a python pkl file that has the full data structure.  Either can be
# used for plotting or any downstream application.
# import os
# 
# pdbs = ["2R99"]
# 
# def run_ringer(pdb_file,mtz_map_file,scaling="sigma",map_label="2FOFCWT_no_fill,PH2FOFCWT_no_fill"):
#     os.system("phenix.python ringer_ne.py %s %s --scaling=%s --show_gui=False --map_label=%s" %(mtz_map_file,pdb_file,scaling,map_label))
# 
#     
# for pdb in pdbs:
#     #fetch the pdb and structure factors
#     os.system("phenix.fetch_pdb --all %s" %pdb)
#     os.system("phenix.cif_as_mtz %s-sf.cif --symmetry=%s.pdb" %(pdb,pdb))
#     #make a 2fofc map for ringer sampling
#     os.system("phenix.model_vs_data map=\"2mFo-DFc\" %s-sf.mtz %s.pdb" %(pdb,pdb))
#     #sample the map using ringer  - show gui is off by default right now, but can be useful to check quickly if things ran ok
#     run_ringer("%s.pdb" %pdb,"%s-sf_2mFobs-DFmodel_map_coeffs.mtz" %pdb , scaling="volume",map_label="2mFobs-DFmodel,P2mFobs-DFmodel")
